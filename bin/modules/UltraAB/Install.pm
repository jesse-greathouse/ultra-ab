...
use Exporter 'import';
our @EXPORT_OK = qw(install install_help node_build install_node);

my $binDir = abs_path(dirname(__FILE__) . '/../../');
my $applicationRoot = abs_path(dirname($binDir));
my $os = get_operating_system();
my $osModule = 'UltraAB::Install::' . $os;
my $nodeVersion = '22.16';
my $npmVersion = '10.9.2';
...
Installs UltraAB and its dependencies. By default, runs a full install unless specific components are requested.
...
sub install {
    printf "Installing UltraAB at: $applicationRoot\n",
...
sub handle_options {
    my $defaultInstall = 1;
    my @components =  ('system', 'node', 'openresty', 'php', 'wordpress', 'composer');
    my %skips;
    my %installs;
...
sub configure_php {
    ...
    str_replace_in_file('__APP_NAME__', 'UltraAB', $phpFpmConfFile);
...
sub node_build {
    ...
    foreach my $frontend (@microfrontends) {
        unless (-d $frontend) {
            warn "Skipping missing frontend directory: $frontend\n";
            next;
        }

        print "\n=================================================================\n";
        print " Building frontend: $frontend\n";
        print "=================================================================\n\n";

        chdir($frontend) or die "Failed to change directory to $frontend: $!";

        # Clean node_modules if present
        if (-d 'node_modules') {
            system('rm', '-rf', 'node_modules');
            command_result($?, $!, "Removing existing node_modules...", "rm -rf node_modules");
        }
        ...
        command_result($?, $!, "Installing dependencies in $frontend...", "npm install");

        # Build project
        system('bash', '-c', "$nvm_env && npm run build");
        command_result($?, $!, "Building $frontend...", "npm run build");
    }

    chdir($originalDirectory) or die "Failed to return to $originalDirectory: $!";
}
