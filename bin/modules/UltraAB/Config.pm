...
my $configurationFileName = '.ultra-ab-cfg.yml';             # Name of the config file
...
sub write_config_file {
    my ($templateFile, $destinationFile, $domain, %cfg) = @_;

    # Sanity check
    die "Missing domain: cannot flatten config without a domain key" unless $domain;
    die "Domain '$domain' not found in config" unless exists $cfg{$domain};
    die "Expected hashref for domain '$domain'" unless ref($cfg{$domain}) eq 'HASH';

    # Extract domain-specific configuration
    my %domainMap = %{ $cfg{$domain} };
    ...
    for my $key (keys %domainMap) {
        my $placeholder = '__' . $key . '__';
        str_replace_in_file($placeholder, $domainMap{$key}, $destinationFile);
    }
}
