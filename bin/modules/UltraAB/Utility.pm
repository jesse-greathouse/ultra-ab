...
sub get_wordpress_version {
    ...
    # Scan for any lines containing an error message
    foreach my $line (@lines) {
        if ($line =~ /^Error:/) {
            if ($line =~ /This does not seem to be a WordPress installation/) {
                die <<"EOF";
...
EOF
            ...
            # ✅ Skip Docker transient error (generic match)
    ...
    # Exit code fallback (non-error output but failed execution)
    if ($exit_code != 0) {
        die "Failed to get WordPress version using wp-cli:\n$output";
    }

    # Assume last non-empty line is the version number
    my $version = trim($lines[-1]);
...
sub wordpress_database_backup {
    ...
    require UltraAB::Config;
    UltraAB::Config->import(qw(get_configuration));
    ...
    my $binDir          = $meta->{BIN} // die "Missing BIN path in configuration";
    my $varDir          = $meta->{VAR} // die "Missing VAR path in configuration";
    my $tmpDir          = $db_cfg->{TMP} // die "Missing TMP path in configuration";
    my $wp_cli_path     = "$binDir/wp";
    ...
    my $mappedTmpBackup = "$applicationRoot/tmp/$backupSlug";

    rmtree($mappedTmpBackup) if -d $mappedTmpBackup;
    make_path($mappedTmpBackup) or die "Could not create temporary backup directory: $mappedTmpBackup";
    chmod 0777, $mappedTmpBackup or die "Could not chmod temporary backup directory: $mappedTmpBackup";

    my $sqlFile = "$tmpBackup/${dbname}-${date}-${rand}.sql";
    my $mappedSqlFile = "$mappedTmpBackup/${dbname}-${date}-${rand}.sql";
    ...
    die "❌ Export failed: missing expected SQL file $mappedSqlFile\n" unless -f $mappedSqlFile;

    my $zip_path = "$backupDir/$backupSlug.zip";
    unlink $zip_path if -e $zip_path;
    make_path($backupDir) unless -d $backupDir;

    my $zip_cmd = join(' ', 'zip', '-j', shell_quote($zip_path), shell_quote($mappedSqlFile));
    ...
    rmtree($mappedTmpBackup);
    print "🧹 Temporary folder removed: $mappedTmpBackup\n";
...
sub splash() {
  print (''."\n");
  print ('+--------------------------------------------------------------------------------------+'."\n");
  print ('| Thank you for choosing ultra-ab                                                      |'."\n");
  print ('+--------------------------------------------------------------------------------------+'."\n");
  print ('| Copyright (c) 2025 Jesse Greathouse (https://github.com/jesse-greathouse/ultra-ab)   |'."\n");
  print ('+--------------------------------------------------------------------------------------+'."\n");
  print ('| ultra-ab is experimental software for demonstration purposes only.                   |'."\n");
  print ('| All redistribution is unauthorized. Copying, reproducing, or using this software in  |'."\n");
  print ('| a non-demonstration setting is expressly prohibited.                                 |'."\n");
  print ('+--------------------------------------------------------------------------------------+'."\n");
